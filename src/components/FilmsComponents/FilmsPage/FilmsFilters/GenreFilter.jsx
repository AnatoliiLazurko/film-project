...
const GenreFilter = ({ isClean, setIsClean }) => {
    ...
    const handleFilter = (selectedOption) => {
        ...
        const genreUrl = selectedOption.toLowerCase();
    ...
    return (
        ...
        <div className={styles["select-container"]} onClick={() => setFilterOpen(!isFilterOpen)} ref={selectRef} key={1}>
            <div className={styles["custom-select"]}>
                <span>
                    {urlGenre && urlGenre !== 'genre=u' && urlGenre.charAt(0).toUpperCase() + urlGenre.slice(1)}
                    ...
                    {optionsGenre.map((option, index) => (
                        <span key={index}>
                            ...
                            {urlGenre && 
                                <p
                                    key={index}
                                    className={`${option.toLowerCase() === urlGenre ? `${styles["selected-option"]}` : `${styles["select-option"]}`}`}
                                    onClick={() => handleFilter(option)}
                                >
                                    {option}
                                </p>

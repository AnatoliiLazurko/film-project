                        ...
                        {rowsGenres.map((row, rowIndex) => (
                            ...
                            {row.map((genre, index) => (
                                <div className={styles["row"]} key={index}>
                                    <NavLink to={`/films/${genre.name.toLowerCase().replace(/ /g, '_')}/studio=u/date=u/popular=u/1`}>{genre.name}</NavLink>
                                </div>
                            ))}
                            </div>
                        ))}
                    ...
                    <div className={styles["columns-container"]}>
                        {rowsStudios.map((row, rowIndex) => (
                            ...
                            {row.map((studio, index) => (
                                <div className={styles["row"]} key={index}>
                                    <NavLink to={`/films/genre=u/${studio.name.toLowerCase().replace(/ /g, '_')}/date=u/popular=u/1`}>{studio.name}</NavLink>
                                </div>
                            ))}
                            </div>
                        ))}
